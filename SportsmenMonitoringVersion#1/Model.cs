...
namespace Sportsmen_Monitoring
{
    ...
    public class Model
    {
        ...
        {
            LoadFromBinaryFile();
            if (Model.Instance.client == null)
                Model.Instance.client = new ServiceConnectServer.ServiceDataClient();
        }
        ...
        public static bool Authorization(string login, string pass)
        {
            try
            {
                var res = Model.Instance.client.Auth(login, pass);
           //     if (res)
            ...
            {
                MessageBox.Show("Нет соединения с сервером. Попытка авторизаваться локально...");
    ...
    public class User
    {
        ...
        /// Id в системе
        /// </summary>
        public Guid AspnetUserId { get { return _aspnetUserId; } }
    ...
    public class Sport
    {
        ...
        public Sport(int id, string name)
        {
            if (id == 0)
            {
                try
                {
                    var res = Model.Instance.client.AddSport(name);
                    _id = res.Id;
                }
                catch (Exception e)
                {
    ...
    public class Discipline
    {
        ...
        public Discipline(int id, string name, Sport sport, Scale scale, string description)
        {
            if (id == 0)
            {
                try
                {
                    var res = Model.Instance.client.AddDiscipline(name, sport.ID, scale.ID, description);
                    _id = res.Id;
                }
                catch (Exception e)
                {
